...
def make_arrow(root, dataset_root, single_plot=False, missing_type=None):
    ...
    caption_root = os.path.join(root, 'caption.json')
    
    with open(f"{root}/split.json", "r") as fp:
        split_sets = json.load(fp)
        
    print("caption_root=",caption_root)
    with open(caption_root, "r") as fp:
        caption_dir = json.load(fp)
    total_genres = []
    for split, samples in split_sets.items():
        ...
        for sample in tqdm(samples):
            ...
            caption=caption_dir['/data/lhy/missing_aware_prompts/datasets/mmimdb/images/'+sample+'.jpeg']
            data = (binary, plots, caption, label, genres, sample, split)
            print(caption)
        ...
        print("table.column_names=",table.column_names)
