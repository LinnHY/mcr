...
class AffineCoupling(nn.Module):
    def __init__(self, in_channel, filter_size=64, affine=True):
        super().__init__()

        self.affine = affine
        #print("filter_size=",filter_size) 64
        #print("in_channel=",in_channel) 768
...
def gaussian_log_p(x, mean, log_sd): #实现了高斯分布的对数概率密度函数（log-pdf）。对于给定的输入 x、均值 mean 和对数标准差 log_sd，该函数计算了输入 x 在指定均值和标准差下的对数概率密度值。
    """print("x.shape=",x.shape)
    print("mean.shape=",mean.shape)
    print("log_sd.shape=",log_sd.shape)"""
...
class Block(nn.Module):
    ...
    def forward(self, input):
        ...
        if self.split:
            ...
            mean, log_sd = zero, zero  # sd gaussian dis
...
class Glow(nn.Module):
    def __init__(
        ...
        for i in range(n_block - 1):
            self.blocks.append(Block(n_channel, n_flow, split=False, affine=affine, conv_lu=conv_lu))
        self.blocks.append(Block(n_channel, n_flow, split=False, affine=affine, conv_lu=conv_lu))
